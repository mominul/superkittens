...
/// Handle POST /signup
pub async fn handle_sign_up_api(
    ...
    let form_fields = validate_form_fields_or_throw_error(
        &api_options.config.sign_up_feature.form_fields,
        &form_fields_raw,
        tenant_id,
    )
    .await?;
    ...
    let result = api_implementation
        .sign_up_post(
            &form_fields,
            tenant_id,
            None, // session — loaded later when account linking is available
            None, // should_try_linking_with_session_user
            api_options,
            user_context,
        )
        .await?;
    ...
    match result {
        super::super::types::SignUpPostResult::Ok { user, .. } => Ok(serde_json::json!({
            "status": "OK",
        ...
        super::super::types::SignUpPostResult::EmailAlreadyExists => Ok(serde_json::json!({
            "status": "EMAIL_ALREADY_EXISTS_ERROR",
        })),
        super::super::types::SignUpPostResult::NotAllowed { reason } => Ok(serde_json::json!({
            "status": "SIGN_UP_NOT_ALLOWED",
            "reason": reason,
        })),
        super::super::types::SignUpPostResult::GeneralError { message } => Ok(serde_json::json!({
